...
describe 'source helper', ->
  ...
  it "should generate source tag with media query", ->
    ...
    tag = cloudinary.source(FULL_PUBLIC_ID, media: media)
  ...
  it "should generate source tag with responsive srcset", ->
    tag = cloudinary.source(FULL_PUBLIC_ID, srcset: common_srcset)
    expect(tag).to.eql(
      "<source srcset='" +
  ...
  it "should generate picture tag", ->
    tag = cloudinary.picture(FULL_PUBLIC_ID, cloudinary.utils.extend({sources: [
      {"max_width": min_width, "transformation": {"effect": "sepia", "angle": 17, "width": min_width}},
      {"min_width": min_width, "max_width": max_width, "transformation": {"effect": "colorize", "angle": 18, "width": max_width}},
      {"min_width": max_width, "transformation": {"effect": "blur", "angle": 19, "width": max_width}}
    ]}, fill_transformation))

    exp_tag =
      "<picture>" +
      "<source media='(max-width: 100px)' srcset='http://res.cloudinary.com/test123/image/upload/c_fill,h_399,w_399/a_17,e_sepia,w_100/sample.jpg'>" +
      "<source media='(min-width: 100px) and (max-width: 399px)' srcset='http://res.cloudinary.com/test123/image/upload/c_fill,h_399,w_399/a_18,e_colorize,w_399/sample.jpg'>" +
      "<source media='(min-width: 399px)' srcset='http://res.cloudinary.com/test123/image/upload/c_fill,h_399,w_399/a_19,e_blur,w_399/sample.jpg'>" +
      "<img src='http://res.cloudinary.com/test123/image/upload/c_fill,h_399,w_399/sample.jpg' height='399' width='399'/>" +
      "</picture>"
