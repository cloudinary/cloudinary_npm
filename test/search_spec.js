...
describe("search_api", () => {
  ...
  describe("integration", function () {
    ...
    it(`should return all images tagged with ${SEARCH_TAG}`, () => cloudinary.v2.search.expression(`tags:${SEARCH_TAG}`)
      .execute()
      .then((results) => {
        expect(results.resources.length).to.eql(3);
      }));
    it(`should return resource ${PUBLIC_ID_1}`, () => cloudinary.v2.search.expression(`public_id:${PUBLIC_ID_1}`)
      .execute()
      .then((results) => {
        expect(results.resources.length).to.eql(1);
      }));
    it('should paginate resources limited by tag and orderd by ascing public_id', () => cloudinary.v2.search.max_results(1).expression(`tags:${SEARCH_TAG}`)
    ...
    it('should include context', () => cloudinary.v2.search.expression(`tags:${SEARCH_TAG}`).with_field('context')
      .execute()
      .then((results) => {
        expect(results.resources.length).to.eql(3);
    ...
    it('should include context, tags and image_metadata', () => cloudinary.v2.search.expression(`tags:${SEARCH_TAG}`)
      .with_field('context')
      .with_field('tags')
