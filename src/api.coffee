...
call_api = (method, uri, params, callback, options) ->
  ...
  request_options = _._.assignIn request_options,
...
transformation_string = (transformation) ->
  if _.isString(transformation)
    ...
    utils.generate_transformation_string(_._.assignIn({}, transformation))
...
exports.delete_resources = (public_ids, callback, options = {}) ->
  ...
  type = options["type"] ? "upload"
  uri = ["resources", resource_type, type]
  call_api("delete", uri, _.assignIn({"public_ids[]": public_ids}, api.only(options, "keep_original", "invalidate")), callback, options)

exports.delete_resources_by_prefix = (prefix, callback, options = {}) ->
  resource_type = options["resource_type"] ? "image"
  type = options["type"] ? "upload"
  uri = ["resources", resource_type, type]
  call_api("delete", uri, _.assignIn({prefix: prefix}, api.only(options, "keep_original", "next_cursor", "invalidate")), callback, options)
...
exports.delete_all_resources = (callback, options = {}) ->
  ...
  type = options["type"] ? "upload"
  uri = ["resources", resource_type, type]
  call_api("delete", uri, _.assignIn({all: yes}, api.only(options, "keep_original", "next_cursor", "invalidate")), callback, options)
...
publishResource = (byKey, value, callback, options={})->
  ...
  options = _.assignIn {resource_type: resource_type}, options
  call_api("post", uri, params, callback, options)
