...
exports.delete_resources = (public_ids, callback, options = {}) ->
  ...
  type = options["type"] ? "upload"
  uri = ["resources", resource_type, type]
  call_api("delete", uri, _.extend({"public_ids[]": public_ids}, api.only(options, "keep_original", "invalidate", "transformations")), callback, options)

exports.delete_resources_by_prefix = (prefix, callback, options = {}) ->
  resource_type = options["resource_type"] ? "image"
  type = options["type"] ? "upload"
  uri = ["resources", resource_type, type]
  call_api("delete", uri, _.extend({prefix: prefix}, api.only(options, "keep_original", "next_cursor", "invalidate", "transformations")), callback, options)

exports.delete_resources_by_tag = (tag, callback, options = {}) ->
  resource_type = options["resource_type"] ? "image"
  uri = ["resources", resource_type, "tags", tag]
  call_api("delete", uri, api.only(options, "keep_original", "next_cursor", "invalidate", "transformations"), callback, options)

exports.delete_all_resources = (callback, options = {}) ->
  resource_type = options["resource_type"] ? "image"
  type = options["type"] ? "upload"
  uri = ["resources", resource_type, type]
  call_api("delete", uri, _.extend({all: yes}, api.only(options, "keep_original", "next_cursor", "invalidate", "transformations")), callback, options)
