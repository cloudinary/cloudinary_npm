...
function getParsedConfigFromEnv(ENV_STR) {
  let conf = {};
...
function enrichConfigWithURIQuery(ENV_URL) {
...
function enrichCloudinaryConfig(parsedConfig) {
...
module.exports = function (new_config, new_value) {
  if ((cloudinary_config == null) || new_config === true) {
    ...
    [CLOUDINARY_ENV_URL, CLOUDINARY_ENV_ACCOUNT_URL].forEach((ENV_URL) => {
      let parsedConfig = getParsedConfigFromEnv(ENV_URL);
      enrichCloudinaryConfig(parsedConfig);
      enrichConfigWithURIQuery(ENV_URL);
    });
  }
