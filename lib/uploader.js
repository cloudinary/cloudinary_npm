...
exports.upload = function upload(file, callback, options = {}) {
  return call_api("upload", callback, options, () => {
    const params = build_upload_params(options);
...
exports.upload_large = function upload_large(path, callback, options = {}) {
  ...
  return exports.upload_chunked(path, callback, { resource_type: 'raw', ...options });
};

exports.upload_chunked = function upload_chunked(path, callback, options) {
  const file_reader = fs.createReadStream(path);
  const out_stream = exports.upload_chunked_stream(callback, options);
...
class Chunkable extends Writable {
  constructor(options) {
    ...
    this.on('finish', () => {
      if (this.active) {
        this.emit('ready', this.buffer, true, () => {
        });
      }
    });
  }
...
exports.upload_chunked_stream = function upload_chunked_stream(callback, options = {}) {
  ...
  const params = build_upload_params(options);
  const chunk_size = options.chunk_size != null ? options.chunk_size : options.part_size;
  const chunker = new Chunkable({
    chunk_size,
  });
  let sent = 0;
  chunker.on('ready', (buffer, is_last, done) => {
    const chunk_start = sent;
    sent += buffer.length;
    options.content_range = `bytes ${chunk_start}-${sent - 1}/${(is_last ? sent : -1)}`;
    params.timestamp = utils.timestamp();
    const finished_part = function (result) {
    ...
    const stream = call_api("upload", finished_part, options, () => [params, {}, buffer]);
    return stream.write(buffer, 'buffer', () => stream.end());
  });
  return chunker;
};

exports.explicit = function explicit(public_id, callback, options = {}) {
  return call_api("explicit", callback, options, () => utils.build_explicit_api_params(public_id, options));
};

// Creates a new archive in the server and returns information in JSON format
exports.create_archive = function create_archive(callback, options = {}, target_format = null) {
  return call_api("generate_archive", callback, options, () => {
    const opt = utils.archive_params(options);
    if (target_format) {
...
exports.text = function text(content, callback, options = {}) {
  return call_api("text", callback, options, () => {
    const textParams = utils.only(options, TEXT_PARAMS);
    const params = {
      timestamp: utils.timestamp(),
      text: content,
      ...textParams,
...
function call_api(action, callback, options, get_params) {
  ...
  const api_url = utils.api_url(action, options);
  const boundary = utils.random_public_id();
  let errorRaised = false;
  const handle_response = function (res) {
    // var buffer;
    if (errorRaised) {
  ...
  const post_data = utils.hashToParameters(params)
    .filter(([key, value]) => value != null)
    .map(
      ([key, value]) => Buffer.from(encodeFieldPart(boundary, key, value), 'utf8'),
    );

  const result = post(api_url, post_data, boundary, file, handle_response, options);
  if (isObject(result)) {
    return result;
  }
...
function post(url, post_data, boundary, file, callback, options) {
  var file_header;
  const finish_buffer = Buffer.from(`--${boundary}--`, 'ascii');
  if ((file != null) || options.stream) {
    const filename = options.stream ? "file" : basename(file);
  ...
  const post_request = https.request(post_options, callback);
  const upload_stream = new UploadStream({ boundary });
  ...
  post_request.setTimeout(options.timeout != null ? options.timeout : 60000, () => {
    timeout = true;
...
exports.direct_upload = function direct_upload(callback_url, options = {}) {
  ...
  params = utils.process_request_params(params, options);
  const api_url = utils.api_url("upload", options);
  return {
...
exports.image_upload_tag = function image_upload_tag(field, options = {}) {
  const html_options = options.html || {};
  const tag_options = extend({
    type: "file",
    name: "file",
