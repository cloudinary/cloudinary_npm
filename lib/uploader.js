...
function call_api(action, callback, options, get_params) {
  ...
  let make_response=function (resolve, reject){
    ...
    if (isObject(result)) {
      if (USE_PROMISES) resolve(result)
      return result;
    }
  }
  
  if (USE_PROMISES) {
    return new Promise(make_response);
  } else {
    // Running make_response with empty resolve and rejects
    make_response(() => {}, () => {})
  }
}
