...
(function() {
  var EncodeFieldPart, EncodeFilePart, Q, SizeChunker, TEXT_PARAMS, UploadStream, _, build_upload_params, call_api, call_tags_api, config, fs, https, path, post, utils;
  ...
  exports.upload_large = function(path, callback, options) {
    ...
    start = function(err, stats) {
      ...
      return exports.upload_large_stream(file_reader, stats.size, callback, options);
    };
    return fs.stat(path, start);
  };

  exports.upload_large_stream = function(in_stream, file_size, callback, options) {
    var chunker, finished_part, finished_part_final, numChunks, out_stream;
    if (options == null) {
      options = {};
    }
    ...
    chunker.on('chunkEnd', function(id, done) {
      ...
      return out_stream.end();
    });
    chunker.on('data', function(chunk) {
      return out_stream.write(chunk.data);
    });
    return in_stream.pipe(chunker);
  };
