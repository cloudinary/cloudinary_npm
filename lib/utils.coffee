...
exports.url = cloudinary_url = (public_id, options = {}) ->
  ...
  if private_cdn
    prefix += (if secure then secure_distribution else "#{subdomain}#{cloud_name}-res.cloudinary.com")
  else
    prefix += (if secure then secure_distribution else "#{subdomain}res.cloudinary.com") + "/" + cloud_name
