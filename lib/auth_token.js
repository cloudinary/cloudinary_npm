...
function escapeToLower(url) {
  return encodeURIComponent(url).replace(/%../g, match => match.toLowerCase());
}
...
module.exports = function (options) {
  ...
  if (options.expiration == null) {
    if (options.duration != null) {
      const start = options.start_time != null ? options.start_time : Math.round(Date.now() / 1000);
  ...
  const toSign = [...tokenParts];
  if (options.url) {
    const url = escapeToLower(options.url);
    toSign.push(`url=${url}`);
  }
  const auth = digest(toSign.join("~"), options.key);
