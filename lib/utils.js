...
(function() {
  var CLOUDINARY_JS_CONFIG_PARAMS, CONDITIONAL_OPERATORS, CONDITIONAL_PARAMETERS, DEFAULT_RESPONSIVE_WIDTH_TRANSFORMATION, LAYER_KEYWORD_PARAMS, _, build_custom_headers, build_eager, config, crc32, crypto, finalize_resource_type, finalize_source, hashToQuery, join_pair, norm_range_value, number_pattern, offset_any_pattern, offset_any_pattern_re, process_if, process_layer, process_video_params, querystring, smart_escape, split_range, textStyle, unsigned_url_prefix, url, utf8_encode, utils, v1_adapter, v1_result_adapter,
    slice = [].slice;
  ...
  exports.url = function(public_id, options) {
    ...
    auth_token = options.auth_token === false ? false : exports.merge(config().auth_token, utils.option_consume(options, "auth_token"));
    ...
    if (sign_url && auth_token) {
      token = utils.generate_auth_token(exports.merge({
        url: url.parse(resultUrl).path
      }, auth_token));
      resultUrl += "?" + token;
    }
    return resultUrl;
