...
(function() {
  var CLOUDINARY_JS_CONFIG_PARAMS, DEFAULT_RESPONSIVE_WIDTH_TRANSFORMATION, LAYER_KEYWORD_PARAMS, _, build_eager, config, crc32, crypto, finalize_resource_type, finalize_source, hashToQuery, hash_query_params, join_pair, norm_range_value, number_pattern, offset_any_pattern, offset_any_pattern_re, process_layer, process_video_params, querystring, smart_escape, split_range, textStyle, unsigned_url_prefix, utf8_encode, utils, v1_adapter, v1_result_adapter,
    slice = [].slice;
  ...
  exports.generate_transformation_string = function(options) {
    var angle, background, base_transformation, base_transformations, border, color, crop, dpr, effect, flags, has_layer, height, j, key, len, named_transformation, no_html_sizes, overlay, param, params, range_value, ref, ref1, ref2, ref3, ref4, ref5, responsive_width, responsive_width_transformation, result, short, simple_params, size, transformations, underlay, value, width;
  ...
  exports.download_archive_url = function(options) {
    ...
    return exports.api_url("generate_archive", options) + "?" + hash_query_params(cloudinary_params);
  };
  ...
  build_eager = function(eager) {
    ...
    return ((function() {
      ...
      ref = Array(eager);
      results = [];
      for (transformation in ref) {
        format = ref[transformation];
        transformation = _.clone(transformation);
        format = transformation.format || format;
        delete transformation.format;
        results.push(_.compact([utils.generate_transformation_string(transformation, true), format]).join("/"));
      }
      return results;
    })()).join("|");
  };
