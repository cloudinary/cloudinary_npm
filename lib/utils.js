...
textStyle = function textStyle(layer) {
...
normalize_expression = function normalize_expression(expression) {
...
process_if = function process_if(ifValue) {
  if (ifValue) {
...
process_layer = function process_layer(layer) {
  var components, format, public_id, re, res, resource_type, result, start, style, text, textSource, type;
...
base64EncodeURL = function base64EncodeURL(url) {
  var ignore;
...
exports.generate_transformation_string = function (options) {
  ...
  if (size) {
    ...
    var _ref = (_size$split = size.split("x"), _size$split2 = _slicedToArray(_size$split, 2), width = _size$split2[0], height = _size$split2[1], _size$split);

    var _ref2 = _slicedToArray(_ref, 2);

    options["width"] = _ref2[0];
    options["height"] = _ref2[1];
  }
  ...
  if (isArray(effect)) {
    ...
    for (key in effect) {
      value = effect[key];
      effect = "".concat(key, ":").concat(value);
    }
  }
  ...
  if (isObject(border)) {
    border = "".concat((ref = border.width) != null ? ref : 2, "px_solid_").concat(((ref1 = border.color) != null ? ref1 : "black").replace(/^#/, 'rgb:'));
  ...
  for (key in options) {
    ...
    delete options[key];
    var_params.push("".concat(key, "_").concat(normalize_expression(value)));
  }
  ...
  if (!isEmpty(variables)) {
    for (l = 0, len1 = variables.length; l < len1; l++) {
      var _variables$l = _slicedToArray(variables[l], 2);

      name = _variables$l[0];
      value = _variables$l[1];
      var_params.push("".concat(name, "_").concat(normalize_expression(value)));
    }
  }
...
exports.updateable_resource_params = function (options) {
...
exports.url = function (public_id) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var api_secret, auth_token, cdn_subdomain, cloud_name, cname, format, i, original_source, prefix, preloaded, private_cdn, resource_type, resultUrl, secure, secure_cdn_subdomain, secure_distribution, shasum, shorten, sign_url, signature, source_to_sign, ssl_detected, to_sign, token, transformation, type, url_suffix, use_root_path, version;
...
finalize_source = function finalize_source(source, format, url_suffix) {
  var source_to_sign;
...
finalize_resource_type = function finalize_resource_type(resource_type, type, url_suffix, use_root_path, shorten) {
  if (type == null) {
    type = 'upload';
  }
...
unsigned_url_prefix = function unsigned_url_prefix(source, cloud_name, private_cdn, cdn_subdomain, secure_cdn_subdomain, cname, secure, secure_distribution) {
...
smart_escape = function smart_escape(string) {
...
exports.api_sign_request = function (params_to_sign, api_secret) {
  ...
  to_sign = sortBy(function () {
    ...
    for (k in params_to_sign) {
      ...
      if (v != null) {
        results.push("".concat(k, "=").concat(utils.build_array(v).join(",")));
      }
    }
...
exports.sign_request = function (params) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
...
exports.webhook_signature = function (data, timestamp) {
...
exports.private_download_url = function (public_id, format) {
  var options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
  var params;
...
exports.zip_download_url = function (tag) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var params;
  params = exports.sign_request({
    timestamp: exports.timestamp(),
...
exports.download_archive_url = function () {
...
exports.download_zip_url = function () {
  var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
...
join_pair = function join_pair(key, value) {
  if (!value) {
    return void 0;
  } else if (value === true) {
    return key;
...
v1_result_adapter = function v1_result_adapter(callback) {
  if (callback != null) {
...
v1_adapter = function v1_adapter(name, num_pass_args, v1) {
  return function () {
    for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
      args[_key] = arguments[_key];
    }

    var callback, options, pass_args;
...
offset_any_pattern = "(".concat(number_pattern, ")([%pP])?"); // Replace with ///(#{offset_any_pattern()})\.\.(#{offset_any_pattern()})///
// After jetbrains fixes bug

offset_any_pattern_re = RegExp("(".concat(offset_any_pattern, ")\\.\\.(").concat(offset_any_pattern, ")")); // Split a range into the start and end values

split_range = function split_range(range) {
  // :nodoc:
...
norm_range_value = function norm_range_value(value) {
  // :nodoc:
  var modifier, offset;
  offset = String(value).match(RegExp("^".concat(offset_any_pattern, "$")));
...
process_video_params = function process_video_params(param) {
  var video;
...
exports.archive_params = function () {
  var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  var ref;
  return {
...
build_custom_headers = function build_custom_headers(headers) {
  var a;
  return function () {
    var j, len, ref, results;
    ref = Array(headers);
...
exports.build_explicit_api_params = function (public_id) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
...
exports.build_streaming_profiles_param = function () {
  var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  var params;
...
build_eager = function build_eager(eager) {
...
hashToQuery = function hashToQuery(hash) {
  var key, v, value;
      ...
      if (isArray(value)) {
        results.push(function () {
          ...
          for (j = 0, len = value.length; j < len; j++) {
            ...
            results1.push("".concat(querystring.escape("".concat(key)), "=").concat(querystring.escape(v)));
          }

          return results1;
        }().join("&"));
      } else {
        results.push("".concat(querystring.escape(key), "=").concat(querystring.escape(value)));
      }
    }
...
exports.jsonArrayParam = jsonArrayParam = function jsonArrayParam(data, callback) {
  if (!data) {
    return;
  }
