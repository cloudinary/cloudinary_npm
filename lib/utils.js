...
(function() {
  ...
  process_layer = function(layer, escapeQuotes) {
    ...
    if (_.isPlainObject(layer)) {
      ...
      if (_.isEmpty(text) && resource_type !== "text") {
        ...
        if (!_.isEmpty(text)) {
          ...
          if (escapeQuotes) {
            text = text.replace(new RegExp("[,/]", 'g'), function(c) {
  ...
  exports.generate_transformation_string = function(options, escapeQuotes) {
    ...
    if (_.isArray(options)) {
      result = (function() {
        ...
        for (j = 0, len = options.length; j < len; j++) {
          ...
          results.push(utils.generate_transformation_string(_.clone(base_transformation), escapeQuotes));
        }
        return results;
      })();
      return result.join("/");
    ...
    if (base_transformations.length !== 0 && _.filter(base_transformations, _.isObject).length > 0) {
      base_transformations = _.map(base_transformations, function(base_transformation) {
        if (_.isObject(base_transformation)) {
          return utils.generate_transformation_string(_.clone(base_transformation), escapeQuotes);
        } else {
