...
(function() {
  ...
  exports.url = exports.cloudinary_url = function(public_id, options) {
    var api_secret, cdn_subdomain, cloud_name, cname, force_remote, format, original_source, prefix, preloaded, private_cdn, resource_type, secure, secure_cdn_subdomain, secure_distribution, shasum, shorten, sign_url, signature, source_to_sign, ssl_detected, to_sign, transformation, type, url, url_suffix, use_root_path, version, _ref, _ref1;
    if (options == null) {
      options = {};
    }
    ...
    if (!force_remote) {
      ...
      if (type === 'asset' && this.metadata["images/" + public_id]) {
        if (!cloudinary.config.static_image_support) {
          return original_source;
        }
        public_id = this.metadata["images/" + public_id]["public_id"];
        if (!format) {
          public_id += path.extname(original_source);
        }
      } else if (type === 'asset') {
        return original_source;
      }
  ...
  finalize_resource_type = function(resource_type, type, url_suffix, use_root_path, shorten) {
    ...
    if (use_root_path) {
      if ((resource_type.toString() === 'image' && type.toString() === 'upload') || (resource_type.toString() === 'images' && (type == null))) {
        resource_type = null;
        type = null;
      } else {
