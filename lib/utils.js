...
(function() {
  var CLOUDINARY_JS_CONFIG_PARAMS, DEFAULT_POSTER_OPTIONS, DEFAULT_RESPONSIVE_WIDTH_TRANSFORMATION, _, as_safe_bool, config, crc32, crypto, finalize_resource_type, finalize_source, norm_range_value, number_pattern, offset_any_pattern, offset_any_pattern_re, process_video_params, querystring, smart_escape, split_range, unsigned_url_prefix, utf8_encode, utils, v1_adapter, v1_result_adapter,
    slice = [].slice;
  ...
  number_pattern = function() {
    return "([0-9]*)\\.([0-9]+)|([0-9]+)";
  };

  offset_any_pattern = function() {
    return "(" + number_pattern + ")([%pP])?";
  };

  offset_any_pattern_re = function() {
    return /((([0-9]*)\.([0-9]+)|([0-9]+))([%pP])?)\.\.((([0-9]*)\.([0-9]+)|([0-9]+))([%pP])?)/;
  };
  ...
  norm_range_value = function(value) {
    var modifier, offset, ref;
    offset = String(value).match(RegExp("^" + offset_any_pattern + "$"));
    if (offset) {
      modifier = (ref = offset[5] != null) != null ? ref : {
        'p': ''
      };
