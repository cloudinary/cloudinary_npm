    ...
    if (includes([200, 400, 401, 403, 404, 409, 420, 500], res.statusCode)) {
      ...
      res.on("end", () => {
        ...
        if (result.error) {
          ...
          result.rate_limit_allowed = parseInt(res.headers["x-featureratelimit-limit"], 10);
          result.rate_limit_reset_at = new Date(res.headers["x-featureratelimit-reset"]);
          result.rate_limit_remaining = parseInt(res.headers["x-featureratelimit-remaining"], 10);
        }
        if (result.error) {
...
exports.resources = function resources(callback, options = {}) {
  ...
  const { type } = options;
  const uri = ["resources", resource_type];
  if (type != null) {
    uri.push(type);
  }
...
exports.resources_by_tag = function resources_by_tag(tag, callback, options = {}) {
  const resource_type = options.resource_type || "image";
  const uri = ["resources", resource_type, "tags", tag];
...
exports.resources_by_context = function resources_by_context(key, value, callback, options = {}) {
  const resource_type = options.resource_type || "image";
  const uri = ["resources", resource_type, "context"];
  const params = only(options, "next_cursor", "max_results", "tags", "context", "direction", "moderations");
  params.key = key;
  if (value != null) {
    params.value = value;
  }
...
exports.resources_by_moderation = function resources_by_moderation(kind, status, callback, options = {}) {
  const resource_type = options.resource_type || "image";
  const uri = ["resources", resource_type, "moderations", kind, status];
  return call_api("get", uri, only(options, "next_cursor", "max_results", "tags", "context", "direction", "moderations"), callback, options);
};
...
exports.resources_by_ids = function resources_by_ids(public_ids, callback, options = {}) {
  ...
  const uri = ["resources", resource_type, type];
  const params = only(options, "tags", "context", "moderations");
...
exports.resource = function resource(public_id, callback, options = {}) {
  const resource_type = options.resource_type || "image";
  const type = options.type || "upload";
  const uri = ["resources", resource_type, type, public_id];
...
exports.restore = function restore(public_ids, callback, options = {}) {
  const resource_type = options.resource_type || "image";
  const type = options.type || "upload";
  const uri = ["resources", resource_type, type, "restore"];
...
exports.update = function update(public_id, callback, options = {}) {
  ...
  const type = options.type || "upload";
  const uri = ["resources", resource_type, type, public_id];
  const params = utils.updateable_resource_params(options);
...
exports.update_upload_preset = function update_upload_preset(name, callback, options = {}) {
  const uri = ["upload_presets", name];
  const params = utils.merge(utils.clear_blank(utils.build_upload_params(options)), only(options, "unsigned", "disallow_public_id"));
...
exports.create_upload_preset = function create_upload_preset(callback, options = {}) {
  const uri = ["upload_presets"];
  const params = utils.merge(utils.clear_blank(utils.build_upload_params(options)), only(options, "name", "unsigned", "disallow_public_id"));
  return call_api("post", uri, params, callback, options);
};
...
exports.update_upload_mapping = function update_upload_mapping(name, callback, options = {}) {
  const params = only(options, "template");
  params.folder = name;
...
function publishResource(byKey, value, callback, options = {}) {
  const params = only(options, "type", "invalidate", "overwrite");
  params[byKey] = value;
  const resource_type = options.resource_type || "image";
  const uri = ["resources", resource_type, "publish_resources"];
  options = extend({
    resource_type,
  }, options);
  return call_api("post", uri, params, callback, options);
...
exports.update_streaming_profile = function update_streaming_profile(name, callback, options = {}) {
  const params = utils.build_streaming_profiles_param(options);
