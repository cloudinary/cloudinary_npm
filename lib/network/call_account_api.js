...
function call_account_api(method, uri, params, callback, options) {
  ...
  const auth = {
    api_key: ensureOption(options, "provisioning_api_key"),
    api_secret: ensureOption(options, "provisioning_api_secret"),
  };

  return make_request(method, params, auth, api_url, callback, options);
}
