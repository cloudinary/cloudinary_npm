...
const PRELOADED_CLOUDINARY_PATH = /^([^/]+)\/([^/]+)\/v(\d+)\/([^#]+)#([^/]+)$/;

class PreloadedFile {
  constructor(file_info) {
    const matches = file_info.match(PRELOADED_CLOUDINARY_PATH);
    if (!matches) {
    ...
    const public_id_and_format = PreloadedFile.split_format(this.filename);
    [this.public_id] = public_id_and_format;
    [this.format] = public_id_and_format;
  }

  is_valid() {
    const expected_signature = utils.api_sign_request({
  ...
  static split_format(identifier) {
    const last_dot = identifier.lastIndexOf(".");
    if (last_dot === -1) {
      return [identifier, null];
    }
    const public_id = identifier.substr(0, last_dot);
    const format = identifier.substr(last_dot + 1);
