...
function chainTransformations(options, transformation = []) {
  ...
  const urlOptions = cloudinary.utils.extractUrlParams(options);
  const currentTransformation = cloudinary.utils.extractTransformationParams(options);
...
exports.image = function image(source, options) {
  const localOptions = _.extend({}, options);
  const srcsetParam = optionConsume(localOptions, 'srcset');
  const attributes = optionConsume(localOptions, 'attributes', {});
  ...
  const client_hints = optionConsume(localOptions, "client_hints", cloudinary.config().client_hints);
  const responsive = optionConsume(localOptions, "responsive");
  const hidpi = optionConsume(localOptions, "hidpi");

  if ((responsive || hidpi) && !client_hints) {
    localOptions["data-src"] = src;
    const classes = [responsive ? "cld-responsive" : "cld-hidpi"];
    const current_class = optionConsume(localOptions, "class");
...
exports.video = function video(public_id, options) {
  ...
  const source_transformation = optionConsume(options, 'source_transformation', {});
  const fallback = optionConsume(options, 'fallback_content', '');

  if (source_types.length === 0) source_types = cloudinary.utils.DEFAULT_VIDEO_SOURCE_TYPES;
  const video_options = _.cloneDeep(options);
  ...
  const multi_source = _.isArray(source_types) && source_types.length > 1;
  let source = public_id;
  if (!multi_source) {
    source = `${source}.${cloudinary.utils.build_array(source_types)[0]}`;
  }
  const src = cloudinary.utils.url(source, video_options); // calculate src and reduce video_options
  if (!multi_source) {
  ...
  if (multi_source) {
    html += source_types.map((source_type) => {
      const transformation = source_transformation[source_type] || {};
      const sourceSrc = cloudinary.utils.url(`${source}.${source_type}`, _.extend({ resource_type: 'video' }, _.cloneDeep(options), _.cloneDeep(transformation)));
      const video_type = source_type === 'ogv' ? 'ogg' : source_type;
...
exports.source = function source(public_id, options = {}) {
  const srcsetParam = cloudinary.utils.extend({}, options.srcset, cloudinary.config().srcset);
...
exports.picture = function picture(public_id, options = {}) {
  ...
  return `<picture>${
    sources.map((source) => {
      const sourceOptions = chainTransformations(options, source.transformation);
