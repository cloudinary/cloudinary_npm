...
function backup_url(action = 'upload', options = {}) {
  let cloudinary = ensureOption(options, "upload_prefix", "https://api.cloudinary.com");
  let cloud_name = ensureOption(options, "cloud_name");
  let asset_id = options.asset_id; // 383f0ff3474741b72fe011d6341f5ef2
  let version_id = options.version_id; //a8811c2d4274d45a8dbb76913738e2ef
  return [cloudinary, "v1_1", cloud_name, action, asset_id, "backup", version_id].join("/");
}
...
exports.download_backedup_resource = function download_backedup_resource(options = {}) {
  let cloudinary_params = exports.sign_request(exports.backup_params(merge(options, {
    mode: "download",
  })), options);
  return exports.backup_url("resources", options) + "?" + hashToQuery(cloudinary_params);
}
...
function backup_params(options = {}) {
  return {
    timestamp: options.timestamp || exports.timestamp(),
  };
}
...
exports.backup_params = backup_params;
