 ...
 */
function process_layer(layer, forUrl = true) {
  var result = '';
  if (isPlainObject(layer)) {
    ...
    if (isEmpty(text) && resource_type !== "text") {
      ...
      if (!isEmpty(text)) {
        ...
        let re = /\$\([a-zA-Z]\w*\)/g;
        let start = 0;
        let textSource = decodeURIComponent(text);
        if (forUrl) {
...
exports.generate_transformation_string = function generate_transformation_string(options, forUrl = true) {
  if(utils.isString(options)) {
    return options;
  }
  if (isArray(options)) {
    return options.map(t=>utils.generate_transformation_string(clone(t), forUrl)).filter(utils.present).join('/');
  }
  ...
  let overlay = process_layer(utils.option_consume(options, "overlay"), forUrl);
  let underlay = process_layer(utils.option_consume(options, "underlay"), forUrl);
