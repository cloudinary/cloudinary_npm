...
const getSDKSuffix = require('../utils/encoding/sdkSuffix/getSDKSuffix');
...
function url(public_id, options = {}) {
  ...
  let analytics = ensureOption(options, 'analytics', true);
  let isResponsive = ensureOption(options, 'responsive', false);

  if (analytics === true) {
    let encodedSDKSuffix = getSDKSuffix({
      isResponsive,
    });
    ...
    resultUrl = `${resultUrl}${appender}a=${encodedSDKSuffix}`;
  }
