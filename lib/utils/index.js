...
function generate_auth_token(options) {
  const token_options = Object.assign({}, config().auth_token, options);
...
function textStyle(layer) {
  const font_family = layer.font_family;
  const font_size = layer.font_size;
  const keywords = [];
  let style = "";
  Object.keys(LAYER_KEYWORD_PARAMS).forEach((attr) => {
    const default_value = LAYER_KEYWORD_PARAMS[attr];
    const attr_value = layer[attr] || default_value;
    if (attr_value !== default_value) {
      keywords.push(attr_value);
    }
  });
  const letter_spacing = layer.letter_spacing;
  if (letter_spacing) {
    keywords.push(`letter_spacing_${letter_spacing}`);
  }
  const line_spacing = layer.line_spacing;
  if (line_spacing) {
...
function normalize_expression(expression) {
  ...
  expression = expression.replace(replaceRE, match => CONDITIONAL_OPERATORS[match] || PREDEFINED_VARS[match]);
    ...
    if (layer.resource_type === "fetch" || (layer.url != null)) {
      ...
      if (isEmpty(text) && resource_type !== "text") {
        ...
        if (!isEmpty(text)) {
          ...
          const re = /\$\([a-zA-Z]\w*\)/g;
          let start = 0;
          const textSource = smart_escape(decodeURIComponent(text), /[,\/]/g);
          text = "";
...
function option_consume(options, option_name, default_value) {
  const result = options[option_name];
...
function generate_transformation_string(options) {
  ...
  const responsive_width = utils.option_consume(options, "responsive_width", config().responsive_width);
  let width = options.width;
  let height = options.height;
  const size = utils.option_consume(options, "size");
  if (size) {
    [width, height] = size.split("x");
    [options.width, options.height] = [width, height];
  }
  const has_layer = options.overlay || options.underlay;
  const crop = utils.option_consume(options, "crop");
  const angle = utils.build_array(utils.option_consume(options, "angle")).join(".");
  const no_html_sizes = has_layer || utils.present(angle) || crop === "fit" || crop === "limit" || responsive_width;
  ...
  const flags = utils.build_array(utils.option_consume(options, "flags")).join(".");
  const dpr = utils.option_consume(options, "dpr", config().dpr);
  if (options.offset != null) {
    [options.start_offset, options.end_offset] = split_range(utils.option_consume(options, "offset"));
  }
  const overlay = process_layer(utils.option_consume(options, "overlay"));
  const underlay = process_layer(utils.option_consume(options, "underlay"));
  const ifValue = process_if(utils.option_consume(options, "if"));
  ...
  simple_params.forEach(([name, short]) => {
    const value = utils.option_consume(options, name);
    if (value !== undefined) {
      params[short] = value;
    }
  });
  ...
  const variablesParam = utils.option_consume(options, "variables", []);
  const variables = entries(options)
  ...
  const raw_transformation = utils.option_consume(options, 'raw_transformation');
  ...
  if (responsive_width) {
    const responsive_width_transformation = config().responsive_width_transformation
...
function url(public_id, options = {}) {
  let signature; let source_to_sign;
  ...
  const format = utils.option_consume(options, "format");
  const cloud_name = utils.option_consume(options, "cloud_name", config().cloud_name);
  if (!cloud_name) {
    throw "Unknown cloud_name";
  }
  const private_cdn = utils.option_consume(options, "private_cdn", config().private_cdn);
  const secure_distribution = utils.option_consume(options, "secure_distribution", config().secure_distribution);
  let secure = utils.option_consume(options, "secure", null);
  const ssl_detected = utils.option_consume(options, "ssl_detected", config().ssl_detected);
  if (secure === null) {
    secure = ssl_detected || config().secure;
  }
  const cdn_subdomain = utils.option_consume(options, "cdn_subdomain", config().cdn_subdomain);
  const secure_cdn_subdomain = utils.option_consume(options, "secure_cdn_subdomain", config().secure_cdn_subdomain);
  const cname = utils.option_consume(options, "cname", config().cname);
  const shorten = utils.option_consume(options, "shorten", config().shorten);
  const sign_url = utils.option_consume(options, "sign_url", config().sign_url);
  const api_secret = utils.option_consume(options, "api_secret", config().api_secret);
  const url_suffix = utils.option_consume(options, "url_suffix");
  const use_root_path = utils.option_consume(options, "use_root_path", config().use_root_path);
  ...
  if (sign_url && isEmpty(auth_token)) {
    let to_sign = [transformation, source_to_sign].filter(part => (part != null) && part !== '').join('/');
    try {
  ...
  let resultUrl = [prefix, resource_type, type, signature, transformation, version, public_id].filter(part => (part != null) && part !== '').join('/');
  if (sign_url && !isEmpty(auth_token)) {
    auth_token.url = urlParse(resultUrl).path;
    const token = generate_token(auth_token);
    resultUrl += `?${token}`;
  }
  return resultUrl;
}
...
function unsigned_url_prefix(
  ...
  if (secure) {
    ...
    const subdomain = cdn_subdomain ? 'a' + ((crc32(source) % 5) + 1) + '.' : '';
    prefix = 'http://' + subdomain + cname;
  } else {
    const cdn_part = private_cdn ? cloud_name + '-' : '';
    const subdomain_part = cdn_subdomain ? '-' + ((crc32(source) % 5) + 1) : '';
    const host = [cdn_part, 'res', subdomain_part, '.cloudinary.com'].join('');
    prefix = 'http://' + host;
  }
  if (shared_domain) {
    prefix += '/' + cloud_name;
...
function smart_escape(string, unsafe = /([^a-zA-Z0-9_.\-\/:]+)/g) {
  return string.replace(unsafe, match => match.split("").map(c => "%" + c.charCodeAt(0).toString(16).toUpperCase()).join(""));
}
...
function api_sign_request(params_to_sign, api_secret) {
  const to_sign = entries(params_to_sign).filter(
    ([k, v]) => utils.present(v),
  ).map(
...
function sign_request(params, options = {}) {
  const apiKey = ensureOption(options, 'api_key');
  const apiSecret = ensureOption(options, 'api_secret');
...
function private_download_url(public_id, format, options = {}) {
  const params = exports.sign_request({
    timestamp: exports.timestamp(),
    public_id,
    format,
    type: options.type,
...
function download_archive_url(options = {}) {
  const cloudinary_params = exports.sign_request(exports.archive_params(merge(options, {
    mode: "download",
  })), options);
...
function html_attrs(attrs) {
  return filter(map(attrs, (value, key) => join_pair(key, value))).sort().join(" ");
}
...
function cloudinary_js_config() {
  const params = utils.only(config(), ...CLOUDINARY_JS_CONFIG_PARAMS);
...
function v1_adapter(name, num_pass_args, v1) {
  return function (...args) {
    const pass_args = take(args, num_pass_args);
...
function v1_adapters(exports, v1, mapping) {
  return Object.keys(mapping).map((name) => {
    const num_pass_args = mapping[name];
...
function generate_responsive_breakpoints_string(breakpoints) {
  ...
  for (let j = 0; j < breakpoints.length; j++) {
    const breakpoint_settings = breakpoints[j];
...
function build_streaming_profiles_param(options = {}) {
  const params = utils.only(options, "display_name", "representations");
