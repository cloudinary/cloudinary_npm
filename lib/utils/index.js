...
function extractTransformationParams(options) {
  return pick(options, ...TRANSFORMATION_PARAMS);
}
...
function cloudinary_js_config() {
  let params = pick(config(), ...CLOUDINARY_JS_CONFIG_PARAMS);
...
function build_streaming_profiles_param(options = {}) {
  let params = pick(options, "display_name", "representations");
...
exports.only = pick; // backwards compatibly, 'only' is publicly exposed from utils (cloudinary.utils.only)
exports.pick = pick;
