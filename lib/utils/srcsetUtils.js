...
function scaledUrl(public_id, width, transformation, options = {}) {
  const configParams = utils.extractUrlParams(options);
...
function generateImageResponsiveAttributes(publicId, attributes = {}, srcsetData = {}, options = {}) {
  ...
  if (generateSrcset || generateSizes) {
    const breakpoints = getOrGenerateBreakpoints(publicId, srcsetData, options);

    if (generateSrcset) {
      const transformation = srcsetData.transformation;
      const srcsetAttr = generateSrcsetAttribute(publicId, breakpoints, transformation, options);
